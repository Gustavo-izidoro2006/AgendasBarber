...
export default function Login() {
  ...
  const isDisabled = carregando || carregandoSessao || !email.trim() || !senha.trim();
        ...
        <form onSubmit={enviar} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div>
            ...
            <div style={{ position: "relative" }}>
              ...
              <input
                type="password" placeholder="Sua senha" value={senha}
          ...
          >
            {carregando || carregandoSessao ? "Entrando..." : "Entrar →"}
          </button>
        </form>

        <div style={{ textAlign: "center", marginTop: 20, fontSize: 14, color: "var(--text-muted)" }}>
          Não tem conta?{" "}
          <Link to="/cadastro" style={{ fontWeight: 600, color: "var(--accent)" }}>Criar agora</Link>
        </div>
      </div>
    </main>
  );
