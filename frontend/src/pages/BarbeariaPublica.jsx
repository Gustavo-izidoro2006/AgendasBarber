  ...
  return (
    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10 }}>
        <div style={{ flex: 1 }}>
          <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 4 }}>{servico.nome}</div>
          {servico.descricao ? (
            <div style={{ fontSize: 13, color: "rgba(255,255,255,0.60)", marginBottom: 6 }}>{servico.descricao}</div>
          ) : null}
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            {duracao && <span style={{ fontSize: 12, color: "rgba(255,255,255,0.70)" }}>⏱ {duracao} min</span>}
          </div>
        </div>
        {valor && (
          <span style={{ fontWeight: 700, fontSize: 16, color: "#FD366E", whiteSpace: "nowrap" }}>{valor}</span>
        )}
      </div>
    </button>
  );
...
export default function BarbeariaPublica() {
  ...
  const valorTotal = servicosSelecionados.length > 0
    ? servicosSelecionados.reduce((sum, s) => sum + (Number(s.valor) || 0), 0)
    : 0;
  ...
  return (
    <main className="bp-main" style={{ minHeight: "100vh", background: "#0a0a0a", color: "white", animation: "fadeSlideUp 0.3s ease" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div className="bp-grid">
          ...
          <aside className="bp-aside" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.10)", borderRadius: 16, display: "grid", gap: 16 }}>
            ...
            {/* Resumo */}
<div style={{ background: "rgba(10,10,10,0.92)", border: "1px solid rgba(255,255,255,0.10)", borderRadius: 12, padding: 14 }}>
              <div style={{ fontSize: 13, color: "rgba(255,255,255,0.60)", marginBottom: 8 }}>Resumo</div>
              <div style={{ fontWeight: 700 }}>{servicosSelecionados.length > 0 ? servicosSelecionados.map(s => s.nome).join(", ") : "—"}</div>
            ...
            {confirmado ? (
              <div style={{ padding: "14px", borderRadius: 12, background: "rgba(29,158,117,0.12)", border: "1px solid rgba(29,158,117,0.30)", fontSize: 14 }}>
                ...
                <div style={{ color: "rgba(255,255,255,0.70)", fontSize: 13 }}>
                  {servicosSelecionados.map(s => s.nome).join(", ")} — {data ? isoParaPT(data) : ""} às {horario?.slice(0,5)}
                </div>
                <button
                  type="button"
